...
async def test_build_faiss_indexes(monkeypatch):
    # Mocking data returned from the database for regular and token-level embeddings
    mock_embedding_data = [("model1", "text1", json.dumps([1.0, 1.0])), ("model1", "text2", json.dumps([1.0, 1.0]))]
    mock_token_embedding_data = [("model1", "token1", json.dumps([1.0, 1.0])), ("model1", "token2", json.dumps([1.0, 1.0]))]
    ...
    async def mock_execute(*args, **kwargs):
        ...
        if "SELECT llm_model_name, token, token_level_embedding_json FROM token_level_embeddings" in args[0]:
            return AsyncMock(fetchall=AsyncMock(return_value=mock_token_embedding_data))()
    ...
    assert "model1" in token_faiss_indexes
    
    # Verify that associated texts have been correctly identified
    assert associated_texts_by_model["model1"] == ["text1", "text2"]
    ...
    assert token_faiss_indexes["model1"].ntotal == len(embedding_array)
